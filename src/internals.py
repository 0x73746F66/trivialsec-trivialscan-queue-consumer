...
class MemberAccount(BaseModel, DAL):
    ...
    def load(
        ...
        if not self.name:
            return
        object_key = f"{APP_ENV}/accounts/{self.name}/registration.json"
        raw = services.aws.get_s3(path_key=object_key)
        if not raw:
            logger.warning(f"Missing account object: {object_key}")
            return
        try:
            data = json.loads(raw)
        ...
        if not data or not isinstance(data, dict):
            logger.warning(f"Missing account data for object: {object_key}")
            return
        super().__init__(**data)
        return self
...
class ConfigDefaults(BaseModel):
    ...
    tmp_path_prefix: str = Field(default=CACHE_DIR)
